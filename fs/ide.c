...
void ide_read(u_int diskno, u_int secno, void *dst, u_int nsecs) {
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		temp = begin + off;
		if (syscall_write_dev(&temp, DEV_DISK_ADDRESS + DEV_DISK_OFFSET, 4) < 0)
		{
			user_panic("ide_read error!");
		}
		temp = DEV_DISK_OPERATION_READ;
		if (syscall_write_dev(&temp, DEV_DISK_ADDRESS + DEV_DISK_START_OPERATION, 4) < 0)
		{
			user_panic("ide_read error!");
		}
		temp = 0;
		if (syscall_read_dev(&temp, DEV_DISK_ADDRESS + DEV_DISK_STATUS, 4) < 0)
		{
			user_panic("ide_read error!");
		}
		if (temp == 0)
