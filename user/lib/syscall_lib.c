...
int syscall_write_dev(void *va, u_int dev, u_int len) {
	...
	msyscall(SYS_write_dev, va, dev, len);
}

int syscall_read_dev(void *va, u_int dev, u_int len) {
	/* Exercise 5.2: Your code here. (2/2) */
	msyscall(SYS_read_dev, va, dev, len);
}

void syscall_set_path(char * path) {
	msyscall(SYS_set_path, path);
}

void syscall_get_path(char* newpath) {
	msyscall(SYS_get_path, newpath);
}
